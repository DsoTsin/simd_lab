...
public:
  ...
  TRefCountPtr &operator=(ReferencedType *InReference) {
    // Call AddRef before Release, in case the new reference is the same as the
    // old reference.
  ...
  FORCEINLINE operator ReferenceType() const { return Reference; }
  ...
  FORCEINLINE void
  Swap(TRefCountPtr
           &InPtr) // this does not change the reference count, and so is faster
  {
    ReferencedType *OldReference = Reference;
    Reference = InPtr.Reference;
...
FORCEINLINE bool operator==(const TRefCountPtr<ReferencedType> &A,
...
template <typename ReferencedType>
FORCEINLINE bool operator==(ReferencedType *A,
                            const TRefCountPtr<ReferencedType> &B) {
